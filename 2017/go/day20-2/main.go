...
func (b *Buffer) Tick() {
	...
	for i, p := range b.particles {
		...
		if _, exists := positions[position]; exists {
			positions[position] = append(positions[position], i)
		} else {
			positions[position] = []int{i}
		}
	}

	// I'm pretty sure there's a better way to do this, but eh
	toDelete := make(map[int]bool)
	for _, members := range positions {
		if len(members) > 1 {
